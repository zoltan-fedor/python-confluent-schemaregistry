...
class CachedSchemaRegistryClient(object):
    ...
    def _send_request(self, url, method='GET', body=None, headers=None):
        ...
        try:
            ...
            code = e.code
            #result = json.loads(e.read().decode("utf8", 'ignore')) # it seems this is not in JSON format for some reason
            result = e.read().decode("utf8", 'ignore')
            print('Error: '+str(code)+', Message: '+str(result))
            #message = "HTTP Error (%d) from schema registry: %s %d" % (code,
            #                                                           result.get('message'),
            #                                                           result.get('error_code'))
            message = "HTTP Error (%d) from schema registry: %s" % (code, result)
